...
import { attachmentUrl } from '../dataLayers/attachmentUtils';
...
export async function getItem(todoId:string, userId: string):Promise<TodoItem> {
    return todosAccess.getItem(todoId,userId)
}
...
export async function createAttachmentPresignedUrl( todoId: string, userId: string):Promise<string> {
    ...
    const uploadUrl = todosAccess.getPresignedUrl(todoId)
    await attachmentUrl(todoId, userId,`https://${bucketName}.s3.amazonaws.com/${todoId}`
    );
    return uploadUrl;
  }